	...
	if(ppid){
		...
		$.ajax({
			...
			success: function (d) {
				...
				/*var flTitel=$(".flText").html();
				if(/\s/.test(flTitel)){
					flTitel=flTitel.split(' ');
					$(".fenleiDiv .yiji li").each(function(i,ele){
						console.log($(this).html(),flTitel[0],flTitel[1])
...
function fenLeiSearch(flid) {
	...
	$.ajax({
		...
		data: {
			...
			brand_id: ppid.id||ppid,
			category_id: flid.id||flid,
			page: pageIndex
		},
		success: function (d) {
			...
			$.each(data1, function (i, d) {
				flStr += '<li class="goodsList"> \
									<a href="goodsDetails.html?rels_id=' + d.info.rels_id + '&count_type='+ d.info.count_type+'">\
										<div class="goodsImg"> \
		...
		success: function (d) {
			...
			if ($(".goodsContent1").hasClass('showAllShop')) {
				...
				$.each(listd, function (i, d) {
					...
					allGoods += '<li class="goodsList"> \
									<a href="goodsDetails.html?rels_id=' + d.info.rels_id + '&count_type='+ d.info.count_type+'" rels_id="'+d.info.rels_id+'" count_type="'+d.info.count_type+'">\
										<div class="goodsImg"> \
											<img src="' + d.images + '" alt=""> \
										</div> \
										<div class="goodsLabel"> \
...
//change-----------------------------------------------------------------------------
	...
	//href="goodsDetails.html?rels_id=' + d.info.rels_id + '&count_type='+ d.info.count_type+'"
					$(this).click(function(){
