...
export const noResetTime1 = function (el, binding) {
    /*let vue=new Vue();
     let count=vue.getCookie('captchaRegist')||60;
     if(vue.getCookie('captchaRegist')){
     RemTime(count);
     }else{
     return;
     }
     function RemTime(count){
     el.innerHTML=count+"s重新获取";
     ...
     el.setAttribute('disabled',true);
     el.style.background="#ccc";
     vue.setCookie('captchaRegist', count, (1 / 86400) * count);
     } else {
     clearInterval(resend);
     el.innerHTML="发送验证码";
     el.removeAttribute('disabled');
     el.style.background="#ed5564";
     vue.removeCookie('captchaRegist');
     }
     }, 1000);
     }*/
};
